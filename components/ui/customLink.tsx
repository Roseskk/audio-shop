...
const CustomLink: React.FC<Props> = ({link, type, text, style}) => {
    return (
        <>
            {
                type === 'transparent' &&
                <Link className={styles.custom_btn + ' ' + styles.transparent} href={link}>{text}</Link>
            }
            {
                type === 'default' &&
                <Link style={{width: style ? style : ''}} className={styles.custom_btn + ' ' + styles.default}
                      href={link}>{text}</Link>
            }
            {
